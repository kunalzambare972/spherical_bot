...
with open('roll_output.csv', mode='w', newline='') as file:
    writer = csv.writer(file)
...
try: 
    while True:
        ...
        roll_data.append((roll_actual, timestamp))
        
        for row in roll_data:
            write_to_csv(row, 'roll_output.csv')
        
        roll_err = set_pt_roll - roll_actual
        user_pwm = 185  # map(y, 0, 65, 0, 200)
        mot_pwm = max(0, min(200, 140 * Kp_roll * roll_err + user_pwm))
        mot_dir = True
